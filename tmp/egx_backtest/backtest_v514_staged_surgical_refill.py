...
INIT=100000.0; HALF=.0025; FRONTIER=v513.FRONTIER; FEATURES=v513.FEATURES
...
def prep(data,trades,p):
 ...
 closes={s:{r['date']:r['close'] for r in rows if p[0]<=r['date']<=p[1]} for s,rows in data.items()}; highs={s:{r['date']:r['high'] for r in rows if p[0]<=r['date']<=p[1]} for s,rows in data.items()}; lows={s:{r['date']:r['low'] for r in rows if p[0]<=r['date']<=p[1]} for s,rows in data.items()}
 timeout_px={(t['symbol'],t['entry_date']):data[t['symbol']][t['entry_i']+7]['close'] for t in arr if t['exit_type']=='timeout'}
 return {'arr':arr,'dates':dates,'by':by,'closes':closes,'highs':highs,'lows':lows,'timeout_px':timeout_px}
...
def simulate(ctx,rule,cfg):
 ...
 def add(s,x,d,target_frac):
  nonlocal cash
  eq,_=mark(d); target_budget=min(x['entry_equity']*target_frac,eq*target_frac); need=max(0,target_budget-x['budget']); amt=min(need,cash)
  if amt>1:
 ...
 for d in ctx['dates']:
  for s in list(pos):
   ...
   # frozen v3.2 structural exit outcome; no new discretionary exit
   if t['exit_date']==d:
    xp=ctx['timeout_px'][(s,t['entry_date'])] if t['exit_type']=='timeout' else e*(1+t['gross_return'])
    cash+=x['shares']*xp*(1-HALF); stats[t['exit_type']]+=1; pos.pop(s); continue
   if not x['flagged']: continue
   cr=px/e-1
   if x['age']==1 and (x['cum_mfe']>=cfg['d1_mfe'] or cr>=cfg['d1_close']):
    add(s,x,d,cfg['restore_frac']); stats['d1_confirm']+=1
   if x['age']==2 and x['cum_mfe']>=cfg['d2_mfe'] and cr>=cfg['d2_close']:
    add(s,x,d,cfg['pyramid_frac']); stats['d2_confirm']+=1
...
def build_rules(train,periods):
 ...
 ranked.sort(key=lambda z:z[0],reverse=True); top=[x[1][0] for x in ranked[:70]]; rules=[]
 for r in top:
 ...
 rules.sort(key=lambda z:z[0],reverse=True); return prim,rules[:180]
...
def main():
 ...
 cfgs=[]
 for initial in [.10,.15,.20,.25,.30]:
  for d1m in [.005,.01,.015,.02]:
   for d1c in [-.01,0.0,.005]:
    for restore in [.45,.50]:
     for d2m in [.015,.025,.035]:
      for d2c in [-.005,0.0]:
       for pyr in [.50,.55,.60]: cfgs.append({'initial_frac':initial,'d1_mfe':d1m,'d1_close':d1c,'restore_frac':restore,'d2_mfe':d2m,'d2_close':d2c,'pyramid_frac':pyr})
 rows=[]; champs=[]
 ...
 res={'version':'v5.14','name':'Staged Surgical Refill','status':'STRICT CHAMPION' if champs else 'NO STRICT CHAMPION','selection_warning':'Historical optimization across already-observed 2023-2026 periods; not pristine out-of-sample. Pre-entry thresholds derive from 2021-2022 quantiles, but rule/config selection uses later outcomes.','frontier':FRONTIER,'dataset':{'stocks':len(data),'signals':len(trades),'primitives':len(prim),'rules':len(rules),'configs_per_rule':len(cfgs),'configs_tested':len(rows)},'strict_count':len(champs),'champion':champs[0] if champs else None,'best_near_miss':best,'top20':rows[:20]}
