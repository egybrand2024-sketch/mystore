...
CONFIRM_RET=[0.0,0.005,0.01]
MIN_WEALTH=1.00; MIN_DD_RED=0.10; MIN_ACTIVE=0.95
NAME="Defensive Lift v5.10B — Two-Day Hold Confirmation"
...
def simulate(trades,data,period,rule,cfg=None):
    ...
    cash=100000.0; pos={}; last={}; curve=[]; real=[]; skipped=0; adds=0; hzentries=0
    def mark(d):
        pv=0.0
    ...
    for d in dates:
        for s in list(pos):
            ...
            if d==p["exit_date"]:
                ...
                real.append({"symbol":s,"exit_type":p["exit_type"],"hazard":p["hazard"],"added":p["added"],"cash_return":proceeds/p["budget"]-1}); pos.pop(s); continue
            if cfg and p["hazard"] and not p["stage_done"] and p["age"]==2:
                p["stage_done"]=True
                if px/p["entry"]-1>=cfg["confirm_ret"]:
                    need=max(0,p["full_budget"]-p["budget"]); add=min(need,cash)
                    if add>1:
                        p["shares"]+=add*(1-a.HALF_FRICTION)/px; p["budget"]+=add; cash-=add; p["added"]=True; adds+=1
        for t in eb.get(d,[]):
            s=t["symbol"]
            if s in pos or len(pos)>=2: skipped+=1; continue
            eq,_=mark(d); hzd=bool(cfg and rule(t)); frac=cfg["initial_frac"] if hzd else .50
            budget=min(eq*frac,cash); full_budget=eq*.50
            if budget<=1: skipped+=1; continue
            shares=budget*(1-a.HALF_FRICTION)/t["entry"]; cash-=budget
            pos[s]={"shares":shares,"budget":budget,"full_budget":full_budget,"entry":t["entry"],"entry_date":d,"exit_date":t["exit_date"],"exit_type":t["exit_type"],"exit_price":a.exit_price(t,data),"age":0,"hazard":hzd,"stage_done":not hzd,"added":False}
            if hzd: hzentries+=1
            last[s]=t["entry"]
    ...
    return {"trades":len(real),"skipped":skipped,"final_equity":final,"total_return":final/100000-1,"cagr":(final/100000)**(1/yrs)-1,"max_drawdown":mdd,"dd_peak":pd,"dd_trough":td,"weekly":v58.weekly(curve),"avg_exposure":v58.mean([x["exposure"] for x in curve]),"hazard_runtime":{"entries":hzentries,"adds":adds},"realized":real,"curve":curve}
...
def main():
    ...
    for f in INITIAL_FRACS:
      for cr in CONFIRM_RET:
        cfg={"initial_frac":f,"confirm_ret":cr}
    ...
    result={"version":"v5.10B","name":NAME,"status":"Eligible" if best["strict_eligible"] else "Rejected / Near Miss","hazard_rule":{"clearance_gte":cthr,"lift_gte":lthr},"architecture":"Hazard trade starts below 50%; no exit and no reduction; add to 50% only after day-2 close confirms persistence.","baseline":{"2023":slim(b23),"2024":slim(b24),"final_research_period":slim(bf)},"grid":{"tested":len(rows),"eligible":len(eligible)},"best":best,"final_result":slim(rf) if rf else None,"top10":rows[:10]}
    ...
    with open("tmp/egx_backtest/results_v510b_two_day_hold.json","w",encoding="utf-8") as f: json.dump(result,f,ensure_ascii=False,indent=2)
