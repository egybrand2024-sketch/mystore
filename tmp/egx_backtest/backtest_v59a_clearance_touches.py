...
NAME="Defensive Lift v5.9A — Clearance + Low Resistance Touches"
...
def main():
    ...
    c_thr=q([t["clearance"] for t in tr],0.75); touch_thr=q([t["resistance_touches"] for t in tr],0.25)
    def rule(t): return t.get("clearance") is not None and t.get("resistance_touches") is not None and t["clearance"]>=c_thr and t["resistance_touches"]<=touch_thr
    ...
    for probe in PROBES:
      for conf in CONFIRMS:
       for act in ACTIONS:
        cfg={"hazard_mode":"v59a","probe_frac":probe,"confirm_mode":conf,"unconfirmed_action":act}
    ...
    with open("tmp/egx_backtest/results_v59a_clearance_touches.json","w",encoding="utf-8") as f: json.dump(result,f,ensure_ascii=False,indent=2)
