...
NAME="Defensive Lift v5.9B — Clearance + Weak Market20"
...
def main():
    ...
    c_thr=q([t["clearance"] for t in tr],0.75); m_thr=q([t["market20_ret"] for t in tr],0.25)
    def rule(t): return t.get("clearance") is not None and t.get("market20_ret") is not None and t["clearance"]>=c_thr and t["market20_ret"]<=m_thr
    ...
    for probe in PROBES:
      for conf in CONFIRMS:
       for act in ACTIONS:
        cfg={"hazard_mode":"v59b","probe_frac":probe,"confirm_mode":conf,"unconfirmed_action":act}
    ...
    with open("tmp/egx_backtest/results_v59b_clearance_market.json","w",encoding="utf-8") as f: json.dump(result,f,ensure_ascii=False,indent=2)
