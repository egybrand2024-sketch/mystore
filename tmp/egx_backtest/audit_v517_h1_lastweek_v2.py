...
WEEK_START='2026-08-30'; WEEK_END='2026-09-03'; INTERVAL='60m'
OUT='tmp/egx_backtest/results_v517_h1_lastweek.json'; CAIRO=ZoneInfo('Africa/Cairo')
def finite(x):
...
def fetch_hourly(symbol):
    ...
    params={'range':'60d','interval':INTERVAL,'includePrePost':'false','events':'div,splits'}
    last_err=None
    for attempt in range(4):
        try:
            r=requests.get(url,params=params,impersonate='chrome',timeout=30)
            if r.status_code!=200:last_err=f'HTTP {r.status_code}: {r.text[:120]}';time.sleep(1+attempt);continue
            obj=r.json();res=((obj.get('chart') or {}).get('result') or [])
            if not res:last_err=str((obj.get('chart') or {}).get('error'));time.sleep(1+attempt);continue
            z=res[0];ts=z.get('timestamp') or [];q=((z.get('indicators') or {}).get('quote') or [{}])[0]
            arrays=[q.get('open') or [],q.get('high') or [],q.get('low') or [],q.get('close') or [],q.get('volume') or []]
            rows=[]
            ...
            ded={x['date']:x for x in rows};return [ded[k] for k in sorted(ded)],None
        except Exception as e:last_err=repr(e);time.sleep(1+attempt)
    return [],last_err
def download(symbols):
    data={};failed=[]
    for k,s in enumerate(symbols,1):
        rows,err=fetch_hourly(s);wr=[r for r in rows if WEEK_START<=r['session_date']<=WEEK_END]
...
def scan_symbol(symbol,rows,market):
    ...
    while t<len(rows):
        ...
        matches=[c for n in range(v3.MIN_BASE,v3.MAX_BASE+1) if (c:=v3.v1_candidate(rows,t,n))]
        if matches:
            c=max(matches,key=lambda x:x['base_n']);s=frozen.live_features(symbol,rows,t,c,market)
            if s['v2_flag']>0:
...
def main():
    ...
    if len(data)<80:
        with open(OUT,'w',encoding='utf-8') as f:json.dump({'experiment':EXPERIMENT,'period':[WEEK_START,WEEK_END],'error':'HOURLY_COVERAGE_TOO_LOW','usable_symbols':len(data),'reference_symbols':len(symbols),'failed_sample':failed[:50]},f,ensure_ascii=False,indent=2)
    ...
    session_dates=sorted(set(r['session_date'] for rows in data.values() for r in rows if WEEK_START<=r['session_date']<=WEEK_END));by_day=[]
    ...
    result={'experiment':EXPERIMENT,'timeframe':'1h','period':[WEEK_START,WEEK_END],'frozen_daily_source_commit':FROZEN_COMMIT,'important_caveat':'Literal DAILY v5.17 numeric rules applied to 1-hour bars; timeframe change is material and this is not a validated H1 strategy.','data':{'reference_symbols':len(symbols),'usable_symbols':len(data),'coverage_ratio':len(data)/len(symbols),'failed_or_insufficient_count':len(failed),'hourly_bars_in_week_across_universe':sum(1 for rows in data.values() for r in rows if WEEK_START<=r['session_date']<=WEEK_END),'source':'Yahoo Finance chart API .CA range=60d interval=60m raw OHLC'},'summary':{'candidates_total':len(signals),'allowed_opportunities':len(allow),'rejected_by_v517_gate':len(reject),'days_with_allowed':len(set(s['session_date'] for s in allow)),'unique_allowed_symbols':len(set(s['symbol'] for s in allow))},'allowed_opportunities':[{'timestamp':s['bar_timestamp'],'date':s['session_date'],'time':s['time'],'symbol':s['symbol'],'signal_close':s['entry'],'research_stop_price':s['research_stop_price'],'research_target_price':s['research_target_price'],'median_base_value':s['median_base_value']} for s in allow],'rejected_signals':[{'timestamp':s['bar_timestamp'],'date':s['session_date'],'time':s['time'],'symbol':s['symbol'],'signal_close':s['entry'],'matched_atoms':s['matched_atoms']} for s in reject],'by_day':by_day,'failed_sample':failed[:40]}
